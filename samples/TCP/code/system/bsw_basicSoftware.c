...
/** The enumeration of all events, tasks and priorities, to have them as symbols in the
    ...
    are dealt out by rtos_osCreateEventProcessor() form the series 0, 1, 2, .... So we
    ...
    rtos_osCreateEventProcessor(). Note, this requires that the order of creating the
    events follows the order here in the enumeration.\n
...
enum
{
    ...
    /** Event for the Ethernet demo: It triggers the task running the IP state machine. The
        event triggers the task on frame Rx (to respond on requests) or regularly by timer
        (to enable proactive requests). */
    idEvTriggerIPProtocolTask,
...
enum
{
    ...
    prioEvTriggerIPProtocolTask,

    prioHighestForApplTasks = prioEv1ms,
};

/** Cross-check publically visible definitions with internally made, actual values. */
_Static_assert( BSW_PRIO_USER_TASK_10MS == prioEv10ms
                &&  BSW_ID_EV_TRIG_INTERNET_PROTOCOL_TASK == idEvTriggerIPProtocolTask
                     ...
                     )
{
    /* The callback into a user process can take only a single (pojnter) argument. We need
       ...
       stack. */
    assert( hMB <= BSW_IDX_LAST_RX_MAILBOX
#if BSW_IDX_FIRST_RX_MAILBOX > 0u
            &&  hMB >= BSW_IDX_FIRST_RX_MAILBOX
#endif
          );
...
int /* _Noreturn */ main(int noArgs ATTRIB_DBG_ONLY, const char *argAry[] ATTRIB_DBG_ONLY)
{
    ...
    eth_osInitEthernetDriver(bsw_ethFrameRxCallback, bsw_ethFrameTxCallback);
    ...
    /* Demonstrate the use of the serial interface. Note, the function is never blocking
       ...
       empty. */
    #define GREETING                                                                    \
      "DEVKIT-MPC5748G - lwIP Demo  Copyright (C) 2023  Peter Vranken\r\n"              \
    ...
    /* Create the events that trigger application tasks at the RTOS. Note, we do not really
       respect the ID, which is assigned to the event by the RTOS API
       rtos_osCreateEventProcessor(). The returned value is redundant. This technique
       requires that we create the events in the right order and this requires in practice
    ...
    CREATE_USER_TASK(idEvTriggerIPProtocolTask, bsw_pidUser, bsw_taskEthernetInternal)

    /* The last check ensures that we didn't forget to register an event. */
    assert(idEventProc == noRegisteredEvents-1);
