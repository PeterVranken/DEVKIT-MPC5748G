...
void loop()
{
    ...
    cli();
    cap_stsTransmission_t stsTransmission_1024 = cap_canBus_PT.PT_StateEcu01_1024_sts.stsTransmission
                        , stsTransmission_1040 = cap_canBus_PT.PT_StateEcu02_1040_sts.stsTransmission
                        , stsTransmission_2032 = cap_canBus_PT.PT_UserLimits_2032_sts.stsTransmission;
    ...
    /* The results are computed in the 10ms task. We need mutual exclusion, when accessing
       the data. */
    cli();
    cap_PT_InfoPowerDisplay_1536_t PT_InfoPowerDisplay_1536 =
                                        cap_canBus_PT.PT_InfoPowerDisplay_1536_sts.signals;
    signed int torque = (int)CAP_PT_1040_TORQUE_BIN_TO_DBL
                                (cap_canBus_PT.PT_StateEcu02_1040_sts.signals.torque)
             , speedOfRotation =
                        (int)CAP_PT_1024_SPEEDOFROTATION_BIN_TO_DBL
                                (cap_canBus_PT.PT_StateEcu01_1024_sts.signals.speedOfRotation);
    signed long power = (long)CAP_PT_1536_POWER_BIN_TO_DBL(PT_InfoPowerDisplay_1536.power);
    sei();
