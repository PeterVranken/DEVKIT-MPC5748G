...
#   srcFileListExcl: a blank separated list of source files excluded from the compilation of
# all *.S, *.c and *.cpp
...
srcFileList := $(call rwildcard,$(srcDirList),*.c *.cpp *.S) $(srcFileListIncl)

# Subtract each excluded file from the list.
srcFileList := $(filter-out $(srcFileListExcl), $(srcFileList))
...
# duplicates.
srcDirInUseList := $(sort $(dir $(srcFileList)))
...
ifneq ($(useFlatObjectFileStructure),)
    ...
    objFileList := $(notdir $(srcFileList))
else
    # Use source file path designation for objcte files, too.
    objFileList := $(srcFileList)
endif
...
export srcDirInUseList srcFileList incDirList objFileList objDirList VPATH
