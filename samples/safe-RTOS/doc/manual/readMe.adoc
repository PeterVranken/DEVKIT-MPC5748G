...
. A countable event with 1 Bit is nearly the same as an ordinary event. but
not exactly. The event processor can be re-triggered once, while the
associated tasks are still ready.
+
...
with one and the same event processor; see _rtos_osCreateEventProcessor_(),
...
_rtos_osCreateSwTriggeredEventProcessor_() is just an "abbreviated" call
of _rtos_osCreateEventProcessor_(): Most of the arguments of the latter
don't care if the event processor is not going to be used for time
triggered task activation. The former variant just sets the unused
...
which is sent with <<secApiSendEvent,_rtos_osSendEvent_()>> or
<<secApiSendEvent,_rtos_sendEvent_()>> or the event multiplicities, if
they are triggered by countable events using
<<secApiSendEvent,_rtos_osSendEventCountable_()>>,
<<secApiSendEvent,_rtos_sendEventCountable_()>>,
<<secApiSendEvent,_rtos_osSendEventMultiple_()>> or
<<secApiSendEvent,_rtos_sendEventMultiple_()>>.
...
The service _rtos_sendEvent_() could easily offer an API, which
