 ...
 * inherit the involved disadvantages (see sio_writeSerial() for details).\n
...
/** The MPC has 18 LINFlex devices. The code is not specific for a certain device but not
    optimal for of all them, neither. The devices 0 and 1 are connected to the DMA and
...
TYP_SECTION(.uncached.data) volatile unsigned long sio_serialOutNoDMATransfers = 0;
...
TYP_SECTION(.uncached.data) volatile unsigned long sio_serialOutNoTruncatedMsgs = 0;
...
TYP_SECTION(.uncached.data) volatile unsigned long sio_serialOutNoLostMsgBytes = 0;
...
/** The ring buffer used for the DMA based serial output.
    ...
    of the buffer.
      @remark The buffer is placed in a linker section, which is specified cache-inhibited.
    This makes the DMA always see the characters written by the CPU. */
static _Alignas(SERIAL_OUTPUT_RING_BUFFER_SIZE) uint8_t TYP_SECTION(.heap.dmaRingBuffer)
...
static TYP_SECTION(.uncached.data) volatile unsigned int _serialOutRingBufIdxWrM = 0;
...
static void configDMA(void)
{
    ...
    extern uint8_t ld_noBitsDmaRingBuffer[0] TYP_DBG_ONLY;
...
# error Configuration error: The chosen DMA channel does not permit timer triggering
    ...
    /* Enable Request Register (DMA_ERQ), RM 70.3.7: Not touched yet, we don't enable the
       channel yet. This will be done in the next use of sio_writeSerial. */
...
#define IDX_LINFLEX_RX_IRQ  (376+3*(IDX_LINFLEX_D))
    ...
    inc_installINTCInterruptHandler( &linFlexRxInterrupt
 ...
 * device plus a DMA channel to serve it. However, to regularly trigger he DMA we still
...
unsigned int sio_writeSerial(const char *msg, unsigned int noBytes)
{
    ...
    static mtx_intercoreCriticalSection_t critSec = MTX_INTERCORE_CRITICAL_SECTION;
