...
Last but not least, this CAN driver has been developed to operate not only
on the MPC5748G but it has been made also for the MPC5775B/E. This
derivative has FlexCAN devices, which don't offer CAN FD at all.
...
the list of #defines to enable one or more of the physically available
FlexCAN devices; see, e.g., #CDR_ENABLE_USE_OF_CAN_0. (Most applications
won't use all of them.) By means of preprocessor switches, the driver
...
all by the driver and remain in reset state and nor do they consume ROM
space for configuration data.
