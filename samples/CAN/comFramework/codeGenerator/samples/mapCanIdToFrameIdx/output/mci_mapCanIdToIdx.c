...
UInt8 mci_getFrameIdxByCanBusAndStdId(UInt8 idxBus, UInt16 canId)
{
    ...
    /* The fields of the table hold 0 for unset pairs and the CAN ID plus one otherwise. We
       ...
       and CAN ID. */
    return (UInt8)(mci_lookupTableFrameIdxByBusAndStdId[idxBus][canId] - 1u);
...
# endif
    ...
    const mci_pairCanIdAndIdx_t keyElement = { .canId.u.id = canId
                                             , .canId.u.isExtId = isExtId
                                             }
    ...
    {
        assert(pFoundElement->canId.u.id == canId
               &&  pFoundElement->canId.u.isExtId == isExtId
              );
