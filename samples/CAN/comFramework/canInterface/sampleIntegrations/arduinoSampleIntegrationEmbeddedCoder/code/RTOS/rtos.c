...
RTOS_PROGMEM_SECTION const char rtos_rtuinosStartupMsg[] = RTOS_RTUINOS_STARTUP_MSG;
...
#ifdef __AVR_ATmega2560__
    /// @todo Here we have a bug: The PC expects the 3-Byte function address but a function
    // pointer like taskEntryPoint only has 16 Bit. Where/How to get the MSByte of the
    // function address? Or at least, how to prove that it is zero (which it will in most
    // situations)? See
    // https://gcc.gnu.org/onlinedocs/gcc/AVR-Options.html#EIND-and-Devices-with-More-Than-128-Ki-Bytes-of-Flash, too.
    // This is what we actually have:
    _Static_assert(sizeof(taskEntryPoint) == 2, "Function pointer size check for 2 Byte");
    // This is what we would like to have (but doesn't compile):
    //_Static_assert(sizeof(taskEntryPoint) == 4, "Function pointer size check for 4 Byte");
    // Consequently, the next line of code doesn't do, what it is expected to. The MSByte
    // of the function address will always be set to zero.
