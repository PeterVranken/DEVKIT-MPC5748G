...
static boolean_t checkForInstallDir(const char *candidate) ATTRIB_UNUSED;
...
static boolean_t checkForInstallDir(const char *candidate)
{
#if 0
...
# define PRINTF(formatStr, ...)
    ...
    char appName[strlen(candidate) + strlen(SUP_APP_FILE_NAME) + sizeof(SL)];
    snprintf(appName, sizeof(appName), "%s" SL SUP_APP_FILE_NAME, candidate);
...
# define PRINTF(...)
    ...
    /* The user should have set an environment variable that points to the installation
       directory. */
    const char * const home = getenv(SUP_ENV_VAR_HOME);
    if(home != NULL  && checkForInstallDir(home))
    {
        PRINTF( "Installation dir %s found via enviroment variable " SUP_ENV_VAR_HOME "\n"
              , home
              );
    ...
    {
        PRINTF("Environment variable " SUP_ENV_VAR_HOME " not set or wrong\n");
    }
