 ...
 *
 * Copyright (C) 2014-2017 Peter Vranken (mailto:Peter_Vranken@Yahoo.de)
 *
...
"     specifies the duration of the simulated time in seconds. Optional, default is\n"      \
"     infinite\n"
#else
# define HELP_TEXT                                                                          \
"usage: " SUP_APP_NAME " [-hrsc] [-v logLevel] [-f headerFormat] [-l[logFileName]]"        \
" [-t[duration]]\n"                                                                         \
"Options:\n"                                                                                \
"  -h, --help\n"                                                                            \
...
static boolean_t checkUserInput( boolean_t parseSuccess
                               ...
                               )
{
    ...
    if(!pCmdLineOptions->help && !pCmdLineOptions->showVersion)
    {
        ...
        if(parseSuccess &&  pCmdLineOptions->tiSimEnd < 0.0)
        {
            parseSuccess = false;
            fprintf( stderr
                   , "Invalid simulation time duration specified. A non negative, valid"
    ...
    if(parseSuccess && pCmdLineOptions->showVersion)
        fprintf(stdout, SUP_APP_NAME " revision " SUP_SW_REV "\n");
...
boolean_t opt_parseCmdLine( opt_cmdLineOptions_t * const pCmdLineOptions
                          , signed int argc
                          , char *argv[]
                          )
{
...
//      }
    ...
    pCmdLineOptions->tiSimEnd = 0.0; /* <= 0 means infinite application run. */
