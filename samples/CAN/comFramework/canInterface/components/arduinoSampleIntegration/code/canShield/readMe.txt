This folder cotains some files, which model the API of a hypothetic CAN
...
handle each one message. Each MTO is either used for reception or for
sending. The MTOs are addressed to by linear, 0 based index. Among some
...
Existing contents of the content buffer are unconditionally overwritten
in an atomic, single clock operation. In the same clock tick are the
status registers updated. An interrupt is raised, which anounces the newly
arrived frame. The interrupt checks, which MTO signalled reception and
...
frame on the physical bus (i.e. about 250 us).
