 ...
 * like bus and frame handles. To resolve these refernces it's likely that you need to
...
#define CDE_COMMON_MACHINE_ALIGNMENT    4
...
/** Some few pieces of code require mutual exclusion of the APSW tasks and the interrupts,
    ...
    such that mutual exclusion is turned on.\n
      It is not necessary, that the implementation supportes nested pairs of enter/leave
    critical section.\n
    ...
    heap. Fortunately, with one exception all heap operations are done during the system
    initialization, i.e. in a still race condition free, single-threaded environment and no
    mutual exclusion is required here.\n
      The exception is using the timer creation API at run-time of the dispatcher engine.
    ...
    interface initialization.\n
      Therefore, if the recommendation of not using the timer creation API at run-time is
    obeyed then the critical section macro should expand to nothing as it is by default. */
...
#define CDE_ENABLE_API_GET_INDEX_BUS            1
...
/** Tailoring of the API: The query function for the transmission direction, \a
    ...
    time. This will safe \a sizeof(boolean_t) Byte per registered frame.\n
      The define is set to either 1 or 0. */
#define CDE_ENABLE_API_IS_INBOUND_TRANSMISSION  1
