...
{
    /// @todo Clean up preliminary code: Make log file and name configurable. Access CAN ID, which should be visible in the log: Code generator can make a map of its frame index to CAN ID and oss has map from osHandle to ifHandle

    if(_logCanData == NULL)
        _logCanData = fopen("canInterfaceDemo.CAN.log", "wt");
    if(_logCanData != NULL)
    {
        fprintf( _logCanData
...
static boolean_t clockMsgSpeed(unsigned char msg[8], unsigned int *pDLC)
{
...
static boolean_t clockMsgTorque(unsigned char msg[8], unsigned int *pDLC)
{
...
static boolean_t clockMsgUserLimits(unsigned char msg[8], unsigned int *pDLC)
{
...
boolean_t oss_sendCanFrame( oss_handleCanFrame_t osHandleFrame
                          , uint8_t *pData
                          , unsigned int DLC
                          )
{
#define P_SEND_ERR 0.01
    ...
    boolean_t success = osHandleFrame != OSS_INVALID_CAN_FRAME_HANDLE;
    assert(success);
