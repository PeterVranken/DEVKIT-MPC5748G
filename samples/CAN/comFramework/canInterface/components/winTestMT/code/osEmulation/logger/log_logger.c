...
/** A global mutex object implements the mutual exclusion to the console, which is shared
    between all execution threads and all existing logger objects. */
static pthread_mutex_t _mutexConsoleStream = PTHREAD_MUTEX_INITIALIZER;
...
/**
 * Lock the access to the core logging functionality. We must not concurrently to a stream
 * from different execution threads. Instead the messages of these threads should be
 * serialized.\n
 *   This lock is related to the console output.
 *   @remark
 * The counterpart is releaseConsoleStream().
 */ 

static inline void lockConsoleStream()
{
    /* We implement the mutex by a pthread mutex object. Other environments may
       require a different implementation. */
#ifdef DEBUG
    int rc =
#endif
    pthread_mutex_lock(&_mutexConsoleStream);
    assert(rc == 0);
   
} /* End of lockConsoleStream */
 ...
 *   This lock is related to the console output.
 *   @remark
 * The counterpart is lockConsoleStream().
 */ 

static inline void releaseConsoleStream()
{
    /* We implement the mutex by a pthread mutex object. Other environments may
       require a different implementation. */
#ifdef DEBUG
    int rc =
#endif
    pthread_mutex_unlock(&_mutexConsoleStream);
    assert(rc == 0);
    
} /* End of releaseConsoleStream */
 ...
 *   This lock is related to the console output.
 *   @param hObj
 * The logger, which the operation relates to.
 *   @remark
 * The counterpart is releaseFileStream().
 */ 

static inline void lockFileStream(log_hLogger_t hObj)
{
    /* We implement the mutex by a pthread mutex object. Other environments may
       require a different implementation. */
#ifdef DEBUG
    int rc =
#endif
    pthread_mutex_lock(&hObj->mutexFileStream);
    assert(rc == 0);
   
} /* End of lockFileStream */




/**
 * Unlock the log file again. We must not concurrently to a stream from different execution
 * threads. Instead the messages of these threads needs to be serialized using this lock.\n
 *   This lock is related to the log file of a given logger object.
 *   @param hObj
 * The logger, which the operation relates to.
 *   @remark
 * The counterpart is lockFileStream().
 */ 

static inline void releaseFileStream(log_hLogger_t hObj)
{
    /* We implement the mutex by a pthread mutex object. Other environments may
       require a different implementation. */
#ifdef DEBUG
    int rc =
#endif
    pthread_mutex_unlock(&hObj->mutexFileStream);
    assert(rc == 0);
    
} /* End of releaseFileStream */
...
void log_initModule()
{
    ...
    int rc =
#endif
    pthread_mutex_init(&_mutexConsoleStream, /* attributes */ NULL);
    assert(rc == 0);
...
void log_shutdownModule()
{
    ...
    int rc =
#endif
    pthread_mutex_destroy(&_mutexConsoleStream);
    assert(rc == 0);
    _mutexConsoleStream = PTHREAD_MUTEX_INITIALIZER;
 ...
 *   @remark
 * This method is not reentrant and must not be called from concurrent execution threads.
 */
                          ...
                          )
{
    ...
    pNewLogger->mutexFileStream = PTHREAD_MUTEX_INITIALIZER;
    ...
    if(fileName != NULL)
    {
        ...
        if(pNewLogger->pLogFile != NULL)
        {
            ...
            int rc =
#endif
            pthread_mutex_init(&pNewLogger->mutexFileStream, /* attributes */ NULL);
            assert(rc == 0);
        }
        else
 ...
 *   @remark
 * This method is not reentrant and must not be called from concurrent execution threads.
 *   @see boolean_t log_createLogger(log_hLogger_t *, const char * const, log_logLevel_t
 *                                  , boolean_t, boolean_t)
...
boolean_t log_close(log_logger_t * const hObj)
{
    if(hObj->pLogFile != NULL)
    {
        ...
        int rc =
#endif
        pthread_mutex_destroy(&hObj->mutexFileStream);
        assert(rc == 0);
        hObj->mutexFileStream = PTHREAD_MUTEX_INITIALIZER;
    }
    else
 ...
 * const, ...)
 *   @remark
 * This method is basically reentrant but the intended use likely isn't: The returned
 * streams are not protected against concurrent use and the output will be fragmented and
 * unreadble if used from concurrent execution threads.
 */
...
{
    /// @todo Check if mutual exclusion is required here. Likely it isn't formally since
    // the C lib call of fflush should be thread-aware but from the functional point of
    // view it might be necessary: flush is likely meant only after a completed log
    // message not somewhere in the middle of one (which is initiated in another context)
 ...
 *   @param addEOL
 * Pass \a true if a newline character should be written after the message.
 *   @param formatString
...
#define MAX_SIZE_LINE_HDR 128
    boolean_t printHeader = hObj->lineFormat != log_fmtRaw  &&  logLevel != log_continueLine;
...
#undef MAX_SIZE_LINE_HDR
    ...
    boolean_t charsCounted = false;

    if(hObj->echoToConsole)
    {
        signed int retVFPrintf;
        lockConsoleStream();
        {
            if(printHeader)
                fputs(lineHeader, stdout);
            retVFPrintf = vfprintf(stdout, formatString, argptr);
    ...
    if(hObj->pLogFile != NULL)
    {
        ...
        lockFileStream(hObj);
        {
            if(printHeader &&  fputs(lineHeader, hObj->pLogFile) < 0)
                hObj->lastFileSystemErr = ferror(hObj->pLogFile);
            retVFPrintf = vfprintf(hObj->pLogFile, formatString, argptr);
            if(retVFPrintf < 0)
                hObj->lastFileSystemErr = ferror(hObj->pLogFile);
            if(addEOL  &&  fputc('\n', hObj->pLogFile) == EOF)
                hObj->lastFileSystemErr = ferror(hObj->pLogFile);
        }
        releaseFileStream(hObj);
 ...
 * which is hidden in a macro.
 *   @remark
 * This function does not append a newline character to the output as boolean_t log_logLine
 * does.
 *   @see boolean_t log_logLine(log_logger_t * const, log_logLevel_t, const char * const, ...)
 */
                    ...
                    )
{
    ...
    unsigned int noChars = logInternal( hObj
                                      , logLevel
                                      , /* addEOL */ false
                                      , formatString
                                      , argptr
 ...
 *   @see boolean_t log_log(log_logger_t * const, log_logLevel_t, const char * const, ...)
 */
